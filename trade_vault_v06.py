...
# Fixes in v.06
# Adding self.balance_history
# Using matplotlib to create a equity graph of closed balances.
...
class Account:
	...
	def plot_balance(self):
		...
		plt.title('Equity Balance Chart')
		plt.show()
		return
...
def graph_creator():
	...
	data_point = [(item["properties"]["Entry Date"]["date"]["start"], item["properties"]["Closing Balance"]["number"])
	              for item in results]

	sorted_data_point = sorted(data_point, key=lambda dp: dp[0])

	dates = []
	balances = []

	for dp_date, dp_balance in sorted_data_point:
		dates.append(dp_date)
		balances.append(dp_balance)

	plt.plot(dates, balances)
	plt.xticks(range(0, len(dates), 10), dates[::10])  # Range determiner.
	plt.xlabel('Dates')
	plt.ylabel('Equity')
	plt.show()
...
def upload_csv_trades(file_path):
	trades = parse_csv_trades(file_path)
	for trade in trades:
		data = {"parent": {"database_id": NOTION_DATABASE_ID},
		        "properties": {
					"Pair": {
...
def main():
	...
	while True:
		try:
			user_prompt = int(
				input(
					'Type 1 to add trades, type 2 to upload historical trades from CSV, type 3 to view your performance, type 4 to view your equity curve, type 5 to quit: '))
			...
			if user_prompt == 1:
				...
				print(
					'\nValues below 0 or above 5 are invalid. \nPick a number between 1, 2, 3, 4 or 5. Please try again.\n')
		# Chose not to break it. Because I want the loop to keep reprompting them to write the correct value. Instead of rerunning it themselves manually.
